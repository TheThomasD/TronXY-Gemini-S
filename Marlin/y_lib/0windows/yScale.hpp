...
class yScale {
	public:
		yScale(int xStart = 0,int yStart = 0,int width = 0,int height = 0,const ScaleAttr* idle_attr = nullptr,const ScaleAttr* focus_attr = nullptr);
		virtual ~yScale(){}
	...
	public:
		...
		void (*fun)(yScale*w,TouchEvent evt);//发生触控事件后,会立即调用本函数
