...
class yPicture : virtual public yScale
{
	public:
		...
		inline virtual uint8_t update(uint8_t force = 0)
		{
			...
			updatePicture(force);//分开写是为继承者灵活调用
			return 0;
		}
		...
		inline virtual uint8_t enUpdate(void) {return yScale::enUpdate() || (picAttr && sign.content.picture == 0);}
		PIC_MODE picMode;
