...
class yWindow {
	public:
		...
		yWindow(int x, int y, int width, int height, yScale *components[], int componentsNum, yWindow* parent = nullptr);
		~yWindow();
		...
		yScale* getComponent(int index){if(index < ComponentNum)return Components[index];return nullptr;}
		int touchDelay;
	...
	private:
		...
		{
			if(yScale::VisualRect.xStart < winRect.xStart)yScale::VisualRect.xStart = winRect.xStart;
			if(yScale::VisualRect.xEnd > winRect.xEnd)yScale::VisualRect.xEnd=winRect.xEnd;
			if(yScale::VisualRect.yStart < winRect.yStart)yScale::VisualRect.yStart = winRect.yStart;
			if(yScale::VisualRect.yEnd > winRect.yEnd)yScale::VisualRect.yEnd = winRect.yEnd;
			return !(yScale::VisualRect.xStart > yScale::VisualRect.xEnd ||
				yScale::VisualRect.yStart > yScale::VisualRect.yEnd);
		}
};
