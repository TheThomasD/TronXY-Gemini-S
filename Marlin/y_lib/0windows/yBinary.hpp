...
class yBinary : virtual public yScale
{
	public:
...
class ySimulation : virtual public yBinary
{
	public:
		...
		inline virtual uint8_t update(uint8_t force = 0)
		{
			...
			updateSimulation(force);//分开是为了继承者灵活调用
			return 0;
		}
		...
		inline virtual uint8_t enUpdate(void) {return yScale::enUpdate() || (simAttr && sign.content.simulation == 0);}
		uint8_t simOpaque;
	protected:
