...
class yWords : virtual public yBinary
{
	...
	protected:
		...
		inline virtual uint8_t enUpdate(void) {return sign.content.text == 0 && (head_word || tail_word || words);}
		int wordsXScale(void);
...
class yLabel : virtual public yWords
{
	public:
		...
		virtual uint8_t update(uint8_t force = 0) {
			if(yScale::update(force))return 1;
			updateWords(force);
			return 0;
		}
		...
		inline virtual uint8_t enUpdate(void) {return yScale::enUpdate() || yWords::enUpdate();}
};
