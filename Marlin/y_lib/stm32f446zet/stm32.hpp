...
#define T_DMA(x)   ((uint32_t)x > (uint32_t)DMA1_Stream7) ? DMA2 : DMA1
...
#define W25Q_RX_DMA_Stream               DMA1_Stream0 //这里更改后,要注意修改完成标志和相应的寄存器及清除标志寄存器
#define W25Q_RX_DMA_Channel              0 //数据流0通道0用于SPI3,见P205
#define W25Q_RX_DMA_TCIF                 DMA_LISR_TCIF0
#define W25Q_RX_DMA_TEIF                 DMA_LISR_TEIF0
#define W25Q_TX_DMA_Stream               DMA1_Stream5
#define W25Q_TX_DMA_Channel              0
void w25q_spi_batch_rcv(uint8_t* buf,uint32_t nbyte);
#ifdef W25Q_RX_DMA_Stream
void w25q_spi_batch_rcv_stop(void);
inline uint8_t w25q_spi_batch_rcv_ok(void) {
	DMA_TypeDef* dma = T_DMA(W25Q_RX_DMA_Stream);
	if(dma->LISR & W25Q_RX_DMA_TCIF)return 1;
	if(dma->LISR & W25Q_RX_DMA_TEIF)return 2;
	return 0;
}
#else
inline void w25q_spi_batch_rcv_stop(void){}
inline uint8_t w25q_spi_batch_rcv_ok() {return 1;}
#endif
