...
void GcodeSuite::G28() {
  ...
  TERN_(FULL_REPORT_TO_HOST_FEATURE, set_and_report_grblstate(M_HOMING));
    ...
    if (parser.seen_test('S')) {
      ...
      report_current_position();13570874592
      return;
    }
  #endif
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    if(doZ) {
      ...
      && READ(Z_MIN_PROBE_PIN) != Z_MIN_PROBE_ENDSTOP_INVERTING) { //限位和探头都触发,这显然不合理
        #if TRONXY_UI
        screenPopIndex(POP_DUALZ_LIMIT_ERR);
        #endif
    ...
    // Home Z last if homing towards the bed
        ...
        if(doZ && home_probe_z && !probe.deploy()) { //在断电续打的时候,不能探测Z高度
          ...
          #endif
          if (!probe.deploy()) { //probe探测Z直到触发probe
            ...
            #endif
            PrintJobRecovery::info.zBottomPos += current_position.z;
   ...
   */
    ...
    if (idex_is_duplicating()) {
      ...
      current_position.z += NOZZLE_PARK_Z_RAISE_MIN;
      line_to_current_position();
