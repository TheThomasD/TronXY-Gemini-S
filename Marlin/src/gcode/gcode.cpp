...
#if USBHOST_HS_EN
#include "stm32f446zet/stm32_usb.hpp"
#endif
  ...
  switch (parser.command_letter) {
      ...
      case 999: M999(); break;                                    // M999: Restart after being Stopped
        ...
        case 8001: //开始打印
          ...
          // else if(planner.has_blocks_queued() || queue.has_commands_queued() || wait_for_heatup)SERIAL_ECHO("M8001-2");
            ...
            my_print_status = PRINT_RUNNING_SERIAL;
            SERIAL_ECHO("M8001-0");
          }
          break;
        case 8002: //暂停
          if(my_print_status == PRINT_RUNNING_SERIAL) {
            ...
            pauseMotion();
            SERIAL_ECHO("M8002-2");
            my_print_status = PRINT_PAUSE_SERIAL;
          }
        ...
        case 8007: //持续进/退料
          if(parser.seen('S'))
          {
            ...
            if(thermalManager.tooColdToExtrude(active_extruder)) {
              if(v == 1)SERIAL_ECHO("M8007-1");
              else SERIAL_ECHO("M8007-2");
              break;
            }
            #endif
            if(v == 1)moveEAxis(active_extruder,true);
            else moveEAxis(active_extruder,false);
            myCurrentFunction = keepEMove;
          }
          break;
        case 8008: //恢复出厂设置
        ...
        case 8009: //设置babystep
          if(parser.seen('S')) {
            ...
            char str[32] = "M8009 V";
            dtostrf(planner.mm_per_step[Z_AXIS] * babystep.accum, 1, 3, &str[7]);
            SERIAL_ECHO(str);
          }
          break;
        case 8886:
          SERIAL_ECHOLNPGM("hotend-num:",hotend_num);
          break;
        case 8887:
          do {
            watchdog_refresh();
            ...
            __NOP();
            auto pa12Init = []() {
              RCC->APB2ENR |= ((uint32_t)0x00000004);//端口A时钟
              GPIOA->CRH &= ~(0xFUL << 16);
              GPIOA->CRH |= 0x4UL << 16;
            };
            auto pg10Init = []() {
              RCC->APB2ENR |= ((uint32_t)0x00000100);//端口G时钟
              GPIOG->CRH &= ~(0xFUL << 8);
              GPIOG->CRH |= 0x4UL << 8;
            };
            pa12Init();
            ...
            for(int j = 0; j < 1000; j++)__NOP();
            uint16_t v = GPIOG->IDR;
            if(v & (1UL << 10))SERIAL_ECHOLNPGM("X:HIGH");
            else SERIAL_ECHOLNPGM("X:LOW");
            v = GPIOA->IDR;
            if(v & (1UL << 12))SERIAL_ECHOLNPGM("Y:HIGH");
            else SERIAL_ECHOLNPGM("Y:LOW");
            __enable_irq();
            #endif
          } while(0);
          break;
    ...
    default:
        ...
        if(parser.command_ptr[0] == ';') {
          ...
          parser.command_ptr[5] == ':') {
            const char* p = parser.command_ptr + 6;
            printTotalTime = yString::strToInt(p,NUM10);
          }
          break;
        }
      #endif
