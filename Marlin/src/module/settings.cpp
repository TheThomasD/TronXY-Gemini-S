...
#define EEPROM_VERSION {2,16,99,26}
...
#include "../core/debug_out.h"
  ...
  const uint8_t version[4] = EEPROM_VERSION;
  ...
  bool MarlinSettings::save() {
    ...
    #if TRONXY_UI
      #if SHOW_MY_VAR_ADDR
      SERIAL_IMPL.print("my_lang-addr:");
      SERIAL_IMPL.println(eeprom_index);
      #endif
      EEPROM_WRITE(my_lang);
      EEPROM_WRITE(my_theme);
      EEPROM_WRITE(buzzer_switch);
      #if SHOW_MY_VAR_ADDR
      SERIAL_IMPL.print("dual_x_mode-addr:");
      SERIAL_IMPL.println(eeprom_index);
      #endif
    ...
    //
    {
      ...
      #if SHOW_MY_VAR_ADDR
      SERIAL_IMPL.print("probe-addr:");
      SERIAL_IMPL.println(eeprom_index);
      #endif
      #if HAS_BED_PROBE
        #if TRONXY_UI
        const xyz_pos_t &zpo = Probe_Offset;
        #else
    ...
    //
    {
      ...
      #if SHOW_MY_VAR_ADDR
      SERIAL_IMPL.print("z-value-addr:");
      SERIAL_IMPL.println(eeprom_index);
      #endif
    ...
    //
    {
      ...
      #if SHOW_MY_VAR_ADDR
      SERIAL_IMPL.print("planner_leveling_active-addr:");
      SERIAL_IMPL.println(eeprom_index);
      #endif
  ...
  bool MarlinSettings::_load() {
    ...
    auto mycmp = [&]() {
      int i = 0;
      for(; i < 4; i++)if(version[i] != stored_ver[i])break;
      return i < 4;
    };
    if(mycmp()) {
      ...
      if (eeprom_error) {
        ...
        DEBUG_ECHO_START();
        DEBUG_ECHOLNPGM("V",version[0],".",version[1],".",version[2],".",version[3]);
  ...
  bool MarlinSettings::load() {
    ...
    void MarlinSettings::load_mesh(const int8_t slot, void * const into/*=nullptr*/) {
        ...
        persistentStore.access_start();
        const uint16_t status = persistentStore.read_data(pos, dest, MESH_STORE_SIZE);
        persistentStore.access_finish();
...
void MarlinSettings::reset() {
  ...
  TERN_(HAS_LEVELING, reset_bed_level());
    ...
    static_assert(COUNT(dpo) == LINEAR_AXES, "NOZZLE_TO_PROBE_OFFSET must contain offsets for each linear axis X, Y, Z....");
      ...
      #if TRONXY_UI
      LOOP_LINEAR_AXES(a) Probe_Offset[a] = dpo[a];
      #else
      LOOP_LINEAR_AXES(a) probe.offset[a] = dpo[a];
      #endif
    #else
      #if TRONXY_UI
      Probe_Offset.set(LINEAR_AXIS_LIST(0, 0, dpo[Z_AXIS], 0, 0, 0));
      #else
