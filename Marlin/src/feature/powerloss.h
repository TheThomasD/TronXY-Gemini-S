...
typedef struct {
  ...
  #if TRONXY_UI
  uint16_t sd_longname[LONG_FILENAME_LENGTH];
  #endif
...
class PrintJobRecovery {
  public:
    ...
    static void load();
    #if TRONXY_UI
    static void save(const bool force=ENABLED(SAVE_EACH_CMD_MODE), const float zraise = 0.0);
    static void clearInterruptSign(/*bool echoInfo*/);
    #else
