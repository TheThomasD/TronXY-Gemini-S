...
void kill(PGM_P const lcd_error/*=nullptr*/, PGM_P const lcd_component/*=nullptr*/, const bool steppers_off/*=false*/) {
  ...
  #endif
  thermalManager.disable_all_heaters(); //不能在上面screenKillError前面,因为报错保存参数时,目标温度会变0
...
void autoLevelEnd(void)
{
	...
	persistentStore.write_data(index,(const uint8_t*)&bilinear_grid_spacing,sizeof(bilinear_grid_spacing));
	persistentStore.write_data(index,(const uint8_t*)&bilinear_start,sizeof(bilinear_start));
	persistentStore.write_data(index,(const uint8_t*)&z_values,sizeof(z_values));
...
const char* software_version = TRONXY_SOFT_VERSION;
